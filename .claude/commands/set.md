...
2. .env.md の存在確認
	2.なければ生成 
    	3. ##記述要望を記載
   ↓
10. 完了報告（作成されたファイルの概要表示）
```

## 記述要望
	### CLUDE.md:
      ```
      - 今後ルール追加時にルールが視覚的にわかりやすくなるように、#======================== を用いて、エリア項目毎に記述する。
      	- 大項目(#項目)につける、小項目(##以降)にはつけない`
      - ユーザーから情報(ID,PASSが必要な実行依頼、環境情報がないと作業できない依頼など...)を用いたアクセス作業・確認を指示されたとき
      	- .env.md を確認する動作になるよう記述する。
