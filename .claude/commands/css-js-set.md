...
### -1. .env.md自動作成（環境情報管理）
プロジェクトルートの `.env.md` を確認し、存在しない場合のみ作成:
...
## 概要
このファイルはプロジェクトの環境情報（ログイン情報、API キー、接続情報など）を管理します。
**機密情報は含めず**、項目のテンプレートとして使用してください。
...
## 注意事項
1. **機密情報の取り扱い**: 実際のパスワードやAPIキーは `.env` ファイルや環境変数で管理
2. **Git管理**: このファイルはテンプレートとして管理、実際の値は `.gitignore` で除外
3. **共有範囲**: チーム内でのみ共有、外部への公開禁止
```

**重要:**
- `.env.md` が既に存在する場合はスキップし、ステップ0（CLAUDE.md更新）へ進む
- 新規作成時のみ上記テンプレートを作成
- 既存ファイルの上書きは行わない
...
**重要**: ユーザーから提供された環境ログイン情報は、必ず項目別に `.env.md` ファイルへ記録してください。

### 記録すべき情報
- **開発/ステージング/本番環境**: URL、管理画面URL、ログイン情報
- **データベース接続情報**: ホスト、DB名、ユーザー名、パスワード、ポート
- **API キー・認証情報**: 各種サービスのAPIキー、シークレット、エンドポイント
- **FTP/SFTP情報**: サーバーアクセス情報
- **その他**: Git リポジトリ、CI/CD設定など

### 記録方法
1. ユーザーから環境情報を受け取った際は `.env.md` を Read
2. 該当するセクションに情報を項目別に記入
3. Edit ツールで更新し、変更を保存
4. 機密性の高い情報は `.env.md` にテンプレートのみ記載し、実値は `.env` ファイルへ

詳細: プロジェクトルートの `.env.md` ファイル参照
```
...
# CSS構築要望時
/css-js-set --css
# → [-1] .env.md作成 → [0] CLAUDE.md作成/更新 → [1] ガイド読み込み → [1.5] docs/自動構築 → [2-4] ガイド準拠実装
...
# CSS + JS 同時構築時
/css-js-set --both
# → [-1] .env.md作成 → [0] CLAUDE.md作成/更新 → [1] 両ガイド読み込み → [1.5] docs/自動構築 → [2-4] ガイド準拠実装
```

## 📝 実装手順詳細

### ステップ-1: .env.md自動作成（初回のみ）

**実行コード例:**
```bash
# プロジェクトルートに.env.mdが存在するか確認
Read: .env.md

# 存在しない場合のみ作成
if (ファイルが存在しない) {
  Write: .env.md
  内容: 環境情報管理テンプレート（ステップ-1セクション参照）
}
...
**作成済みの場合:**
- エラーなく `Read: .env.md` が成功する
- 既存内容を保持し、上書きしない
- ステップ0（CLAUDE.md更新）へ自動的に進む

**新規作成時:**
- 上記テンプレート（ステップ-1セクション）の内容で `.env.md` を作成
- 作成後、ステップ0へ進む

### ステップ0: CLAUDE.md自動構築・更新（初回のみ）
...
# 存在しない場合
Write: CLAUDE.md
内容: CSS/JS構築ガイドライン + 環境情報管理セクション

# 既存でセクションが無い場合
if (「CSS/JS構築ガイドライン」セクションが無い) {
  Edit: CLAUDE.md
